                                       ...
                                       AudioBufferList *               ioData)
{
    ...
    NSLog(@"MYAudioQueueProcessingTapCallback");
...
- (void)failWithErrorCode:(AudioStreamerErrorCode)anErrorCode
{
	@synchronized(self)
	{
		...
		else
		{
            ...
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"error" message:[AudioStreamer stringForErrorCode:anErrorCode] delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
            [alert show];
		}
...
- (double)calculatedBitRate
{
	...
	if (bitRate)
	{
        NSLog(@"bitRate %u",(unsigned int)bitRate);
	...
	eventType:(CFStreamEventType)eventType
{
	...
	else if (eventType == kCFStreamEventHasBytesAvailable)
	{
		...
		@synchronized(self)
		{
            ...
            static long sumByte = 0;
            
			length = CFReadStreamRead(stream, bytes, kAQDefaultBufSize);
            
            sumByte = sumByte +length;
			...
			(state == AS_STOPPED && stopReason == AS_STOPPING_TEMPORARILY))
		{
			...
			if (state == AS_FLUSHING_EOF || buffersUsed == kNumAQBufs - 1)
			{
				...
				else
				{
                    ...
                    UInt32 flags =kAudioQueueProcessingTap_PreEffects;
                    ...
                    if (err)
                    {
                        NSLog(@"AudioQueueProcessingTapDispose erro");
                        return;
                    }
                    //////
...
//    NSLog(@"numberOfChannels %f",numberOfChannels);//    !!!both  44100.000000
    ...
    UInt32 bufferSizeBytes = 4096 * sizeof(Float32);
    UInt32 channels =1;
...
//    ] 0mDataByteSize     16384
//    2014-04-29 13:51:37.452 AudioVisualizer[3836:a103] 1sizeof(Float32)   4
//    2014-04-29 13:51:37.452 AudioVisualizer[3836:a103] 2mDataByteSize     4096
//    2014-04-29 13:51:37.453 AudioVisualizer[3836:a103] 3mNumberChannels   1
                     ...
                     AudioBufferList *ioData)
{
    ...
    OSStatus error= AudioQueueProcessingTapGetSourceAudio(
                                                          inAQTap,
                                                          inNumberFrames,
                                                          ioTimeStamp,
                                                          ioFlags,
...
//    NSLog(@"1111111Callback    %p ",ioData);
//    NSLog(@"mNumberBuffers     %u ",(unsigned int)ioData->mNumberBuffers);
    
        AudioStreamer* owner = (__bridge AudioStreamer *)(inClientData);
        [owner inputCallback:inAQTap
              inNumberFrames:inNumberFrames
                 ioTimeStamp:ioTimeStamp
                     ioFlags:ioFlags
             outNumberFrames:outNumberFrames
                      ioData:ioData
         ];
...
- (void)inputCallback:(AudioQueueProcessingTapRef)inAQTap
          inNumberFrames:(UInt32)inNumberFrames
          ioTimeStamp:(AudioTimeStamp *)ioTimeStamp
          ioFlags:(UInt32 *)ioFlags
          outNumberFrames:(UInt32 *)outNumberFrames
...

{
    ...
    if (error ==noErr)
    {
        ...
        {
            AudioBuffer *input = &ioData->mBuffers[i];
...
//            for (UInt32 frame = 0; frame < *outNumberFrames; frame++)
//            {
