...
- (void)processAudioInput:(id)input allChannels:(BOOL)allChannels
{
    
    
    MicrophoneStream *  inputSampleStream  =(MicrophoneStream *)input;
    if (allChannels)
    {
        // Retrieve waveforms from channels and average these.
        [inputSampleStream.ringBuffers.firstObject copyTo:_inputBuffer length:_pointNumber];
        for (NSUInteger i = 1; i < inputSampleStream.ringBuffers.count; i++)
            [[inputSampleStream.ringBuffers objectAtIndex:i] vectorAverageWith:_inputBuffer index:i length:_pointNumber];
        
        // Fourier transform.
        [self processWaveform:_inputBuffer samleRate:inputSampleStream.sampleRate];
    }
    else
    {
        [self processAudioInput:inputSampleStream channel:0];
    }
}

- (void)processAudioInput:(id)input channel:(NSUInteger)channel
{
    // Retrieve a waveform from the specified channel.
    
    MicrophoneStream *  inputSampleStream  =(MicrophoneStream *)input;

    [[inputSampleStream.ringBuffers objectAtIndex:channel] copyTo:_inputBuffer length:_pointNumber];
    
    // Fourier transform.
    [self processWaveform:_inputBuffer samleRate:inputSampleStream.sampleRate];
}

- (void)processAudioInput:(id)input channel1:(NSUInteger)channel1 channel2:(NSUInteger)channel2
{
    // Retrieve waveforms from the specified channel.
    MicrophoneStream *  inputSampleStream  =(MicrophoneStream *)input;

    [[inputSampleStream.ringBuffers objectAtIndex:channel1] copyTo:_inputBuffer length:_pointNumber];
    [[inputSampleStream.ringBuffers objectAtIndex:channel2] vectorAverageWith:_inputBuffer index:1 length:_pointNumber];
